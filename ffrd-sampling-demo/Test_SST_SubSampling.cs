...
namespace Test_FFRD
{
    ...
    public class Test_SST_SubSampling
    {
        ...
        /// <summary>
        /// I have improved this by rejecting k-NN samples that do not fall within the desired strata.
        /// </summary>
        [TestMethod]
        public void Test_kNN_SubSampling_ToyProblem_V2()
        {
            // Marginal distributions that represent target flow sites
            var dists = new Normal[] { new Normal(10, 2), new Normal(30, 15), new Normal(17, 5), new Normal(99, 14), new Normal(68, 7) };
            ...
            for (int i = 0; i < bins; i++)
            {
                ...
                // In this update, we only include the values that fall within the correct strata
                double zLower = Normal.StandardZ(strata[i].LowerBound);
                double zUpper = Normal.StandardZ(strata[i].UpperBound);
                ...
                for (int d = 0; d < D; d++)
                {
                    for (int k = 0; k < K; k++)
                    {
                        // Check if the sampled value falls within the correct strata
                        double z = xZ[neighborIndices[k], d];
            ...
            for (int d = 0; d < D; d++)
            {
                ...
                var x = CreateXValues(minMax[0, d], minMax[1, d], 100, true);
                // 9b) Compute AEP curve via Total Probability
                var aepX = CreateFrequencyCurve(x, weights, kValues.GetColumn(d), results[d]);
                // 9c) Interpolate to target AEPs
                curves.Add(InterpolateFrequencyCurve(aeps, aepX, true));
        ...
        /// </summary>
        [TestMethod]
        public void Test_kNN_SubSampling_Kanawha_14_sites_V2()
        {
            // 1) Load raw CSV into jagged array: columns: [ID, ..., site flows...]
            // Change file location as needed
            var data = ReadCsvFile("C:/Users/Q0RMCCHS/Documents/0_RMC_Work/FFRD/Kanawha Basin Results/simulation_peaks_by_event_primary.csv");
        ...
        public double[,] CreateFrequencyCurve(double[] xValues, double[] weights, int[] kValues, double[,] samples)
        {
            ...
            for (int i = 0; i < D; i++)
            {
                for (int j = 0; j < N; j++)
                {
                    ...
                    for (int k = 0; k < M; k++)
                    {
                        if (!double.IsNaN(samples[j, k]) && samples[j, k] > xValues[i])
                        {
                            exceed++;
                        }
                    }
                    // Accumulate weighted exceedance probability
                    // Update AEP using Total Probability
                    curve[i, 0] += weights[j] * exceed / kValues[j];
                }
                curve[i, 1] = xValues[i];
            }
